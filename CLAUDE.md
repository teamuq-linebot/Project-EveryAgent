...
```
src/
  main/          主行程（Node + Electron）→ @src/main/CLAUDE.md
  ...
  renderer/      React 渲染行程 → @src/renderer/CLAUDE.md
  shared/        main ↔ preload ↔ renderer 共享型別
    ...
    cliRegistry.ts     — CliId（claude/codex/antigravity）描述子
...
- `src/main/services/agentConversationService.ts` — 一條 claude 對話後端（node-pty 裸 shell；兩段時序注入 300ms + 1500ms）
...
- `src/main/monitor/sessionLiveness.ts` — session 存活偵測（心跳 + ~/.claude/sessions）
...
- `src/main/worktime/claude/` — discover / index / parse（JSONL 純函式 leaf）
